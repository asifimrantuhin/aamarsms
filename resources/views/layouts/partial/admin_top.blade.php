<div class="br-header">
    ...
    <div class="br-header-right">
      ...
      {{-- Data Source --}}
      @php
      $notification = App\Models\Notification::where(['status'=>1,'admin'=>1])->orderby('id','DESC')->paginate(5);
      @endphp
      {{-- Data Source --}}
        ...
        <div class="dropdown">
          ...
          <div class="dropdown-menu dropdown-menu-header wd-300 pd-0-force">
            <div class="d-flex align-items-center justify-content-between pd-y-10 pd-x-20 bd-b bd-gray-200">
              <a href="{{url('admin/notify')}}" <label class="tx-12 tx-info tx-uppercase tx-semibold tx-spacing-2 mg-b-0">Notifications</label></a>
              @if(count($notification) > 0)
              <a href="{{url('admin/notification')}}" class="tx-11">Mark All as Read</a>
              @endif
            </div><!-- d-flex -->
              ...
              @foreach($notification as $not)
                <a href="" class="media-list-link read">
                  <div class="media pd-x-20 pd-y-15">
                    <img src="{{asset('images/user/'.DB::table('users')->whereId($not->user_id)->pluck('image')->first())}}" class="wd-40 rounded-circle">
                    <div class="media-body">
                      <p class="tx-13 mg-b-0 tx-gray-700"><strong class="tx-medium tx-gray-800"></strong> {{$not->text}}</p>
                      <span class="tx-12">{{ date('d-m-y h:i:s',strtotime($not->created_at))}}</span>
                    </div>
                  </div><!-- media -->
                </a>
              @endforeach
            ...
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-header wd-200">
                    <ul class="list-unstyled user-profile-nav">
                        <li><a href="{{url('admin/profile')}}"><i class="icon ion-ios-person"></i> Profile</a></li>
