                ...
                <table id="table_id" class="table table-bordered mg-b-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{$t->created_at}}</td>
                            <td>
                                <a href="{{url('/user/edit_template/'.$t->id)}}" class="btn btn-outline-primary btn-icon mg-r-5">
...
<div id="modaldemo3" class="modal fade">
    <div class="modal-dialog modal-lg" role="document">
        <form class="form-horizontal" method="POST" action="{{ url('user/template/store') }}" enctype="multipart/form-data">
            {{ csrf_field() }}
...
// Sms Counter
    ...
    // Delete Function
        $('#delete').on('show.bs.modal', function (e) {
            ...
            $.ajax({
                type: 'get',
                url: '{{ url('/user/viewtemplate/') }}/' + groupid,
                success: function (template) {
                    $('#name').val(template[0].name);
                    $('#text').val(template[0].text);
                    $("#deletetemp").attr('href', '/user/delete_template/' + template[0].id);
                }
            });
        });
